<script lang="ts">
	...
	import { memoFlag, remainHint } from '../store/sudoku';

	const dispatch = createEventDispatcher();
</script>

<div class="wrapper">
	<div class="item" on:click={() => dispatch('remove')}>
	...
	<div class="item memo" class:active={$memoFlag} on:click={() => ($memoFlag = !$memoFlag)}>
		...
		<div class="sub-text">{$memoFlag ? 'ON' : 'OFF'}</div>
		<div class="text">Memo</div>
	</div>
	<div class="item hint" on:click={() => dispatch('hint')}>
		<svg class="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 33">
			<path
				fill="currentColor"
				fill-rule="evenodd"
		...
		<div class="sub-text">{$remainHint}</div>
		<div class="text">Hint</div>
	</div>
	<div class="item new-game" on:click={() => dispatch('newGame')}>
