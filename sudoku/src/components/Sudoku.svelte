<script lang="ts">
	...
	import { getFocusCellsBox, getFocusCellsCol, getFocusCellsRow } from '../utils/sudoku';
	...
	function isError(selectedPoint: { x: number; y: number }): boolean {
		...
		return (
			rowValues.filter((x) => x === value).length > 1 ||
			...
			{#each answer as row, y (y)}
				<tr>
					{#each row as item, x (x)}
						...
						>
							<Cell value={item} memo={memo[y][x]} on:click={() => (selectedPoint = { x, y })} />
						</td>
					{/each}
				</tr>
			{/each}
